...
class Category extends CI_Controller{
	...
	public function save_category(){
		$this->form_validation->set_rules('category_name','Category Name','required|unique|min_length[2]');
		if($this->form_validation->run()){
		$this->CategoryModel->add_category_model();
	$this->session->set_flashdata("flsh_msg","<font class='success'>Thêm danh mục thành công</font>");
	...
	$this->session->set_flashdata("flsh_msg","<font class='success'>Thêm danh mục phụ thành công</font>");
	...
	public function delete_category($category_id){
		...
		$this->session->set_flashdata("flsh_msg","<font class='success'>Xóa danh mục thành công</font>");
           redirect('category-list');
	}
	public function delete_sub_category($sub_category_id){
		$this->CategoryModel->delete_sub_caegory_by_id($sub_category_id);
		$this->session->set_flashdata("flsh_msg","<font class='success'>Xóa danh mục phụ thành công</font>");
           redirect('sub-category-list');
	}
	...
	public function update_category($category_id){
		...
		$this->session->set_flashdata('flsh_msg','Cập nhật danh mục thành công',10);
	...
	public function update_sub_category($sub_category_id){
		$this->form_validation->set_rules('sub_category_name','Sub Category Name','required|unique|min_length[2]');
		if($this->form_validation->run()){
		$this->CategoryModel->update_sub_caegory_by_id($sub_category_id);
		$this->session->set_flashdata('flsh_msg','Xóa danh mục phụ thành công',10);
