...
class Checkout extends CI_Controller {
	...
	public function customer_registration(){
 	...
 	if($this->form_validation->run()){
		...
		$mdata['subject'] = "Đăng ký thành công......";
	...
	public function customer_login(){
		...
		if($cus_pass==$user_details->cus_password){
			...
			$this->session->set_flashdata('flash_msg','Email hoặc mật khẩu không chính xác...!');
	...
	public function account_page(){
		...
		$customer_id= $this->session->userdata("cus_id");
		$data['cus_info'] = $this->CheckoutModel->select_customer_info_by_id($customer_id);
		$data['main_content'] = $this->load->view('front/account_page',$data,true);
		$this->load->view('front/index',$data);
	}
	...
	public function update_billing(){
		 $this->form_validation->set_rules('cus_mobile', 'Số điện thoại', 'trim|required');
		 $this->form_validation->set_rules('cus_address', 'Địa chỉ', 'trim|required|min_length[5]');
		 $this->form_validation->set_rules('cus_city', 'Quốc gia', 'trim|required');
		 $this->form_validation->set_rules('cus_zip', 'Zip', 'trim|min_length[4]');
		if($this->form_validation->run()){
	...
	public function place_order(){
		...
		$mdata['from'] = "admin@sumon-it.com";
		$mdata['admin_full_name'] = "sumon-it.com";
		$mdata['subject'] = "Đặt hàng thành công......";
		...
		// end Order successfull  mail 
		$this->cart->destroy();
				redirect('order-success2');
				
			}
		}else{
			$this->session->set_flashdata("flash_msg","<font class='btn-warning alert alert-danger'>Vui lòng chọn một phương thức thanh toán</font>");
			redirect("payment");
		}
	}
	...
	public function order_success2(){
		$data =array();
		$data['slider'] = $this->load->view('front/slider','',true);
		$data['recommended'] = $this->load->view('front/recommended','',true);
		$data['main_content'] = $this->load->view('front/order_success2','',true);
		$data['category_brand'] = $this->load->view('front/category','',true);
		$this->load->view('front/index',$data);
	}
