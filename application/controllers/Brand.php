...
class Brand extends CI_Controller{
	...
	public function save_brand(){
		$this->form_validation->set_rules('brand_name','Brand Name','required|unique|min_length[2]');
		if($this->form_validation->run()){
		$this->BrandModel->add_brand_model();
	$this->session->set_flashdata("flsh_msg","<font class='success'>Thêm kiểu dáng thành công</font>");
           redirect('brand-list');
		}else{
			$this->add_brand_form();
		}
	...
	public function delete_brand($brand_id){
		...
		$this->session->set_flashdata("flsh_msg","<font class='success'>Xóa kiểu dáng thành công</font>");
           redirect('brand-list');
	}
	...
	public function update_brand($brand_id){
		...
		$this->session->set_flashdata('flsh_msg','Cập nhật kiểu dáng thành công',10);
